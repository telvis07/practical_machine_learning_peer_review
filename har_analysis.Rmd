...
varImpPlot(modFit$finalModel, 
           scale=FALSE, 
           n.var=10,
           main=sprintf("Top 10 of %s Variables with Gini > 1.0", length(rf_important_varnames)))
```
...
# summary
total_columns_with_all_na_data <- sum(apply(pml_testing, 2, function(x) sum(is.na(x)) == 0))
total_rows_with_NA <- sum(apply(pml_testing, 1, function(x) sum(is.na(x)) == 0))
